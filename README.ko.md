...
메인 목록은 학회 정책과 연구 역할을 기준으로 선별합니다. 이 카탈로그는 더 넓은 범위에서 첫 번째 benchmark 묶음을 모아, 학회 논문·Findings·워크숍·프리프린트의 데이터셋과 평가 프로토콜을 비교할 수 있게 합니다. 이 중 4편은 메인 목록에도 있으므로 여기서는 교차 색인만 제공하며 중복 집계하지 않습니다.
