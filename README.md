...
For the main index, non-survey papers must be formally published in a selected top/main venue: **ACL, EMNLP, NAACL, AAAI, SIGIR, or CIKM**. Surveys are the only exception. Workshop papers, Findings volumes, industry tracks, LREC papers, and preprint-only work are excluded. General RAG, generic agents, role-playing, and traditional recommendation papers are also out unless they explicitly study user-specific adaptation.
